...
export function drawWhsHearthFrame(
  ...
  const mouthW = 28;
  const mouthH = 28;
  fillRoundedRect(ctx, STONE_OUTLINE, 1, px(mouthX - 1), py(mouthY - 1), sc(mouthW + 2), sc(mouthH + 2), sc(6));
  fillRoundedRect(ctx, BACK_BLACK, 1, px(mouthX), py(mouthY), sc(mouthW), sc(mouthH), sc(5));

  // Warm pulse wash inside mouth.
  fillEllipse(ctx, GLOW_WARM, 0.2 * frame.emberGlow,
    px(cx), py(mouthY + mouthH * 0.72), sc(mouthW + 4), sc(mouthH * 0.7));

  // ── Logs — two crossed shapes at base of mouth ──
  fillRoundedRect(ctx, LOG_DARK, 1, px(mouthX + 3), py(mouthY + mouthH - 10), sc(mouthW - 6), sc(6), sc(2));
  fillRoundedRect(ctx, LOG_MID, 1, px(mouthX + 4), py(mouthY + mouthH - 9), sc(mouthW - 8), sc(3), sc(1));
  // Charred log ends.
  ...
  fillTriangle(ctx, LOG_DARK, 1,
    px(mouthX + 5), py(mouthY + mouthH - 3),
    px(mouthX + mouthW - 5), py(mouthY + mouthH - 6),
    px(mouthX + mouthW - 5), py(mouthY + mouthH - 2));
  fillTriangle(ctx, LOG_DARK, 1,
    px(mouthX + 5), py(mouthY + mouthH - 3),
    px(mouthX + 5), py(mouthY + mouthH - 6),
    px(mouthX + mouthW - 5), py(mouthY + mouthH - 6));
  ...
  fillTriangle(ctx, FLAME_BASE, 1,
    px(cx - 9), py(flameBaseY),
    px(cx + 9), py(flameBaseY),
    px(flameX), py(flameTopY));
  fillTriangle(ctx, FLAME_MID, 1,
    px(cx - 6), py(flameBaseY - 1),
    px(cx + 6), py(flameBaseY - 1),
    px(flameX), py(flameTopY + 3));
  fillTriangle(ctx, FLAME_HI, 1,
    px(cx - 3), py(flameBaseY - 2),
    px(cx + 3), py(flameBaseY - 2),
    px(flameX), py(flameTopY + 6));
  fillTriangle(ctx, FLAME_TIP, 1,
    px(flameX - 1.4), py(flameTopY + 8),
    px(flameX + 1.4), py(flameTopY + 8),
    px(flameX), py(flameTopY + 4));

  // Side licks.
  fillTriangle(ctx, FLAME_MID, 1,
    px(cx - 12), py(flameBaseY),
    px(cx - 7), py(flameBaseY),
    px(cx - 10), py(flameBaseY - 6 + frame.leftLickY));
  fillTriangle(ctx, FLAME_MID, 1,
    px(cx + 7), py(flameBaseY),
    px(cx + 12), py(flameBaseY),
    px(cx + 10), py(flameBaseY - 6 + frame.rightLickY));
  fillTriangle(ctx, FLAME_HI, 1,
    px(cx - 11), py(flameBaseY - 1),
    px(cx - 8), py(flameBaseY - 1),
    px(cx - 10), py(flameBaseY - 4 + frame.leftLickY));
  fillTriangle(ctx, FLAME_HI, 1,
    px(cx + 8), py(flameBaseY - 1),
    px(cx + 11), py(flameBaseY - 1),
    px(cx + 10), py(flameBaseY - 4 + frame.rightLickY));
...
function drawKettleOnHob(
  ...
  fillRoundedRect(ctx, KETTLE_BODY, 1, px(kx + 0.6), py(ky - bh + 0.6), sc(bw - 1.2), sc(bh - 1.2), sc(1.5));
  fillEllipse(ctx, KETTLE_HI, 0.85, px(kx + 3.4), py(ky - bh + 2.4), sc(4), sc(1.3));
  fillRectS(ctx, KETTLE_RIM_GLOW, Math.max(0.35, emberGlow * 0.85), px(kx + 1), py(ky - bh + 0.4), sc(bw - 2), sc(0.7));
  ...
  fillRectS(ctx, KETTLE_OUTLINE, 1, px(lidX + (bw - 4) / 2 - 0.8), py(lidY - 1.4), sc(1.8), sc(1.6));
  fillRectS(ctx, KETTLE_HI, 0.7, px(lidX + (bw - 4) / 2 - 0.4), py(lidY - 1.2), sc(0.6), sc(0.6));

  // Spout
  fillTriangle(ctx, KETTLE_OUTLINE, 1,
    px(kx - 2), py(ky - bh + 2),
    px(kx + 1), py(ky - bh + 1),
    px(kx + 1), py(ky - bh + 4));
  fillTriangle(ctx, KETTLE_BODY, 1,
    px(kx - 1.4), py(ky - bh + 2.4),
    px(kx + 0.8), py(ky - bh + 1.8),
    px(kx + 0.8), py(ky - bh + 3.4));
...
function fillRectS(
  ctx: WhsHearthContext, color: string, alpha: number,
