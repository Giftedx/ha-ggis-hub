...
describe('formatDoorStatus', () => {
  it('keeps the fallback text when no door is active', () => {
    expect(
      formatDoorStatus(BASE_SNAPSHOT, HUB_GAME_REGISTRY, 'reduced motion · the bothy bides quiet')
    ).toBe('reduced motion · the bothy bides quiet');
  });

  it('announces the launchable door with every supported interaction key', () => {
    expect(formatDoorStatus(snapshotWithInteraction('launchable', 0), HUB_GAME_REGISTRY)).toBe(
  ...
  it('falls back when the interaction points outside the snapshot doors', () => {
    expect(
      formatDoorStatus(snapshotWithInteraction('launchable', 99), HUB_GAME_REGISTRY, 'fallback')
    ).toBe('fallback');
  });
  ...
  it('uses the door id as the title when the registry has no matching game', () => {
    const snapshot: DecodedSnapshot = {
      ...
      doors: [
        {
          id: 'unregistered-door',
          status: 'locked',
          bounds: { minX: 0, minY: 0, maxX: 100, maxY: 100 },
        },
      ],
    };
    expect(formatDoorStatus(snapshot, HUB_GAME_REGISTRY)).toBe(
      'unregistered-door door — comin’ soon.'
    );
  });
});
