...
End-to-end functional and deployed at `https://ha.ggis.xyz/`. Rust workspace (`hub-core`, `hub-wasm`, `hub-hardlang`), WASM boundary, TypeScript/Vite host, Canvas2D bothy renderer with painted WebP backdrop and procedural fallback, Wee Chieftain haggis mascot, opt-in hub music (two MP3 tracks, `preload=none`), self-hosted Old Standard TT serif font. Hub-owned persistence on a shared versioned-envelope codec (`src/app/versioned-record.ts`): `ggis_hub_settings` (ADR-0007, schema 2 adds the chap-sound preference) and the `ggis_hub_save` progress record (ADR-0008 — visits, chap rotation, door entries; host-side only, sim untouched). Hand-rolled WebAudio chap-knock SFX, gesture-bound and default-on with a persisted opt-out (ADR-0009). Browser smokes on chromium (8, incl. a11y) + firefox + webkit (7 core each). Full gate matrix via `tools/haggis-eval`: rust, rust-cov (llvm-cov 100% lines/fns), docs (report/doc claim drift), ts, coverage (vitest v8 100% all metrics), security, perf, browser, multi-browser, determinism, visual, a11y, soak, supply-chain (cargo deny + machete + gitleaks + osv-scanner), differential rng, differential hash. 100% TS and Rust line/function coverage enforced. Prettier formatting and docs-claim gates in `pnpm verify`. FNV-signed tamper-evident JSON reports (keyless FNV-1a, not cryptographic signatures), two-tier CI in `.github/workflows/ci.yml`, deploy headers + manifest + OG card in `public/`. The plan slice history is in [`docs/plans/2026-05-22-implementation-sequence.md`](docs/plans/2026-05-22-implementation-sequence.md); the running ledger of changes is in [`CHANGELOG.md`](CHANGELOG.md).
