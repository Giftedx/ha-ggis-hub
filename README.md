...
- **~96 KB total client** (59 KB JS + 28 KB WASM + 3.6 KB HTML + 5.3 KB CSS, ~35 KB gzipped) for a Rust+WASM playable hub with deterministic core, self-hosted serif font, opt-in hub music, and cryptographically signed eval reports.
...
- **vitest suite** + cargo workspace tests + five Playwright smokes (keyboard launch, touch tap, pointer-drive touch-drag, music toggle, a11y) + per-asset perf budgets + determinism smoke (same seed + scripted input → same state hash across two browser runs) + a visual gate (perceptual aHash of the canvas at a fixed seed + fixed animation phase, Hamming-distance check against a recorded golden) + a hand-rolled accessibility gate (26 WCAG 2.2 AA spot-checks via Playwright, no axe-core dep).
