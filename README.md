...
- Hub-owned settings persistence: `ggis_hub_settings` uses schema 2 with FNV-digested `music` and `sfx` sections and does not touch WHS keys. Corrupt or unavailable browser storage falls back to defaults.
- Hub-owned progress persistence: `ggis_hub_save` uses schema 1 with FNV-digested `visits`, `lockedChaps`, and `doorEntries` fields. The record is host-side only and leaves the Rust sim untouched.
