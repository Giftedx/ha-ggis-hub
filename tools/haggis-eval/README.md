...
| `browser`             | `node scripts/run-browser-smokes.mjs` — build → vite preview → door-launch + door-tap + pointer-drive + music-toggle + reduced-motion + a11y on chromium → teardown |
| `multi-browser`       | Same runner with `PLAYWRIGHT_BROWSER=firefox` then `webkit`. Runs 5 core smokes (a11y excluded — keyboard focus ordering for anchors is OS-dependent on non-chromium). Requires `playwright install firefox webkit`. |
