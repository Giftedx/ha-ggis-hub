...
func usage(w *os.File) {
	...
	fmt.Fprintln(w, "  all                        Every wired gate; FNV-signed tamper-evident JSON report")
	fmt.Fprintln(w, "")
...
Create `tools/haggis-eval/internal/report/report.go`:
```go
// Package report builds the FNV-signed tamper-evident JSON report that `haggis-eval all`
...
FNV-signed tamper-evident JSON report. Single binary, single source tree, standard library
only.
...
git commit -m "feat(haggis-eval): add Go orchestrator CLI with rust/ts/differential gates and FNV-signed report"
```
...
4. `internal/report/` builds an FNV-signed tamper-evident JSON report; the `Signature` field is the FNV-1a digest of the report payload, verifiable independently.
