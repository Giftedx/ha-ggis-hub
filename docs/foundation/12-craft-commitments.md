...
- **Role**: orchestrate the full eval set into one signed report. Runs Rust headless evals (`cargo nextest` against tagged eval targets), Playwright browser evals, performance-budget checks (`size-limit`, Lighthouse), and the WASM differential test. Aggregates results, prints a human report, writes a machine-readable JSON report, exits non-zero on any failure.
- **Size**: ~500–1500 LOC.
