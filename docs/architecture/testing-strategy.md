...
The pyramid is fully realised as of 2026-05-24. The vitest suite, cargo workspace tests, eight chromium smokes (`scripts/smoke-door-launch.mjs`, `smoke-door-tap.mjs`, `smoke-pointer-drive.mjs`, `smoke-music-toggle.mjs`, `smoke-sfx-toggle.mjs`, `smoke-reduced-motion.mjs`, `smoke-locked-door.mjs`, `smoke-a11y.mjs`), determinism/replay smokes (`smoke-determinism.mjs` for same-seed state-hash equality and `smoke-replay-log.mjs` for browser `.haggislog` capture → WASM `replay_run` hash equality), a perceptual visual gate (`smoke-visual-gate.mjs` against `tests/golden/`), per-asset perf budgets (`scripts/perf-budgets.mjs`), a docs/report claim drift guard (`scripts/check-doc-claims.mjs`), a hand-rolled paint-timing gate (`scripts/smoke-paint-timing.mjs` reading the W3C Paint Timing API + a `hub:firstFrame` user-mark via Playwright; no Lighthouse dep), a hand-rolled accessibility gate (`scripts/smoke-a11y.mjs` running 38 WCAG 2.2 AA spot-checks via Playwright, including reflow and target sizes; no axe-core / pa11y dep), a memory-growth soak (`scripts/smoke-soak.mjs` running the RAF loop 15s, forcing GC via CDP before/after, asserting heap growth < 5 MB; no leak-detection dep), and deploy-headers assertions (`scripts/deploy-config.test.ts`). All orchestrated by `tools/haggis-eval` and run in CI.

## Testing pyramid for this project

```text
Many: Rust unit/property tests for deterministic core              ✓ shipped
Many: TypeScript unit tests for pure host logic                    ✓ shipped
