...
The pyramid is fully realised as of 2026-05-26. 175 vitest cases across 24 files, cargo workspace tests, three chromium smokes (`scripts/smoke-door-launch.mjs`, `smoke-door-tap.mjs`, `smoke-pointer-drive.mjs`), a determinism smoke (`smoke-determinism.mjs`), a perceptual visual gate (`smoke-visual-gate.mjs` against `tests/golden/`), per-asset perf budgets (`scripts/perf-budgets.mjs`), a hand-rolled paint-timing gate (`scripts/smoke-paint-timing.mjs` reading the W3C Paint Timing API + a `hub:firstFrame` user-mark via Playwright; no Lighthouse dep), a hand-rolled accessibility gate (`scripts/smoke-a11y.mjs` running 22 WCAG 2.2 AA spot-checks via Playwright; no axe-core / pa11y dep), a memory-growth soak (`scripts/smoke-soak.mjs` running the RAF loop 15s, forcing GC via CDP before/after, asserting heap growth < 5 MB; no leak-detection dep), and deploy-headers assertions (`scripts/deploy-config.test.ts`). Authored bothy-host decisions that used to sit in browser orchestration now live in `src/hub/bothy-runtime.ts` and are unit-tested, including input packing, URL parsing, coordinate mapping, launch decisions, listener cleanup, event-listener wrapping, and pointer-capture teardown; `src/hub/bothy-module.ts` is excluded only for browser mount orchestration (WASM init, RAF loop, real DOM/canvas integration), while Playwright covers the user-visible launch and pointer flows. All orchestrated by `tools/haggis-eval` and run in CI.

## Testing pyramid for this project

```text
Many: Rust unit/property tests for deterministic core              ✓ shipped
Many: TypeScript unit tests for pure host logic                    ✓ shipped (count asserted by `pnpm verify`)
