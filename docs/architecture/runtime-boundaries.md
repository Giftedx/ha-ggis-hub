...
- Current: `src/hub/room.ts` owns the boundary handle, decodes the initial snapshot, forwards ticks, and re-renders the last snapshot. `src/hub/bothy-module.ts` handles input sampling and interaction data arrives inside the decoded snapshot.
